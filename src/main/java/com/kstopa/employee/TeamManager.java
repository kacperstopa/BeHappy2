...
import com.kstopa.task.UnitsOfWorkComparator;
...
public class TeamManager extends AbstractEmployee implements Manager {
    ...
    public void fire(Employee employee) {
        if (subordinates.contains(employee)) {
            ...
            if(employee instanceof Manager) {
                ...
                subordinates.addAll(manager.getSubordinates());
                subordinates.sort(new UnitsOfWorkComparator());
            }
        }
        else throw new IllegalArgumentException("Can't fire that employee");
    }
    ...
    public void assign(Task task) {
        if ( subordinates.size() == 0 ) throw new IllegalArgumentException("Manager have no developers to assign a task");
        else {
